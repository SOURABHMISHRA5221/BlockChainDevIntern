...
contract CryptoHuman is Ownable{
    ...
    mapping(uint => address) internal _indexToOwner;
    mapping(address => uint) internal _ownerToHumanCount;
    ...
    /*Put 0.01 ether to get new Human with dummy parents and self given gender and generation of first level*/
    function _payEtherForHuman(string memory gender) external payable returns (bool success){
        require(equal(gender,"male")||equal(gender,"female"),"gender should be either male or female");
        require(msg.value == 1 ether,"Please give 1 ether");
        uint gene = _createGene(0,1);
        Human memory human = Human(gene,gender,0,1,block.timestamp,1,block.timestamp);
        population.push(human);
        uint id = population.length-1;
        _indexToOwner[id] = msg.sender;
        _ownerToHumanCount[_indexToOwner[id]]++;
        emit _newHuman(id,human.gene,human.gender);
        return true;
     }
